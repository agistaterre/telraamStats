...
- Méthodes considérées : Prophet, Spline Cubique, SPline Linéaire.
...
p3 <- crit_splin_lin %>%  group_by(crit,type,gap) %>% summarise(na = mean(num_na)) %>% ggplot(aes(x = as.double(gap), y = na, color = type)) + geom_line()  + ggtitle("NA moyen en fonction de maxgap") + xlab("maxgap") + ylab("MAE")
...
spline_lin_hour <- function(data, maxgap) {
  ...
  for (d in unique(data$weekday)) {
    for (h in unique(data$hour)) {
      data_hour <- data %>% filter(weekday == d, hour == h) %>% mutate(vehicle = car + heavy)
      if (nrow(data_hour) > 0) {
        y_hat <- zoo::na.approx(data_hour$vehicle, maxgap = maxgap,na.rm=F)
...
```{r}
zoo::na.approx(c(1,2,3,4,5,NA,NA,NA,4,3,2,2), maxgap = 1 ,na.rm=F)
```
