...
```{r}
pacman::p_load(tidyverse,telraamStats,lubridate,forecast,prophet,purrr,zoo,gridExtra)
...
spline_lin_glob <- function(data, maxgap) {
  ...
  res <- data.frame(weekday = data$weekday,hour = data$hour ,ds = data$ds, y_hat = y, maxgap = maxgap)
  return(res)
}
```
...
spline_lin_hour_glob <- function(data, maxgap_hour,maxgap_glob) {
  res <- data.frame()
 data<- spline_lin_glob(data,maxgap_glob) %>% rename(y = y_hat)
  for (d in unique(data$weekday)) {
    for (h in unique(data$hour)) {
      data_hour <- data %>% filter(weekday == d, hour == h)
      if (nrow(data_hour) > 0) {
        y_hat <- zoo::na.approx(data_hour$y, maxgap = maxgap_hour,na.rm=F)
        res <- rbind(res, data.frame(ds = data_hour$ds, y_hat = y_hat))
      }
    }
  }
...
Evaluate_SplineLin <- function(data, val_reel, rangegap) {
  ...
  for (gap in rangegap) {
    ...
    #  result_hourly_global <- spline_lin_hour_glob(data,gap,3)
  ...
  #  result_hourly_global <- val_reel %>% left_join(result_hourly_global, by = "ds") %>% mutate(error = y - y_hat)
    ...
    #crit_hourly_global <- data.frame(gap = gap, MAE = mean(abs(result_hourly_global$error), na.rm = TRUE), RMSE = sqrt(mean(result_hourly_global$error^2, na.rm = TRUE)), num_na = sum(is.na(result_hourly_global$y_hat)), type = "hourly_global")

    # Ajouter les erreurs dans un dataframe pour chaque gap
    res_model <- rbind(res_model, crit_hourly, crit_daily, crit_global, crit_weekday)
  #  res_model <- rbind(res_model, crit_hourly, crit_global,crit_hourly_global)
  }
...
for (sensor in sensors) {
  ...
  data_imput_list <- Create_test_data_suite(quarterly_to_hourly(sensor), prop_missing=0.20,seed=1234)
  data_imput <- data_imput_list$test   %>% rename(ds=date,y=vehicle)
  date_imput <- data_imput$ds[which(is.na(data_imput$y))]
  val_reel <- data_imput_list$temoin  %>% filter(date %in% date_imput) %>% select(date,vehicle,hour,weekday) %>% rename(ds=date,y=vehicle)
...
p3 <- crit_splin_lin %>%  group_by(type,gap) %>% summarise(na = mean(num_na)) %>% ggplot(aes(x = as.double(gap), y = na, color = type)) + geom_line()  + ggtitle("NA moyen en fonction de maxgap") + xlab("maxgap") + ylab("Nombre de NA")
grid.arrange(p2,p3,ncol=2)
```

```{r}
retrieve_missing_data(data_13)
...
- Fonction 
```{r}
imput_rolling_median <- function(data, window) {
  imputed_data <- data
...
imput_rolling_mean <- function(data, window) {
  imputed_data <- data
...
results_combined <- results_hourly
...
end_time_glob <- Sys.time()
print(end_time_glob - star_time_glob)
...
# Tracer les courbes
p1 <- results_hourly %>% ggplot(aes(x = window, y = mae_mean, color = model)) +
  geom_line() +
  labs(
    title = "Comparaison du MAE en fonction de la taille de la fenêtre",
    x = "Taille de la fenêtre",
  ...
  scale_color_manual(values = c("Hourly" = "green")) +
  theme_minimal()

p2 <- results_hourly %>% ggplot(aes(x = window, y = na_mean)) +
  geom_line() +
  labs(
    title = "Comparaison du nombre de NA en fonction de la taille de la fenêtre",
    x = "Taille de la fenêtre",
...
crit_MA_data <- rbind(crit_MA_data, results_hourly)
grid.arrange(p1, p2, ncol = 2)
list_graph_rolling[[sensor$segment_name[1]]] <- grid.arrange(p1, p2, ncol = 2)
...
# Modèle journalier/heure
rolling_hourly <- function(data, window) {
  
  # Créer une colonne pour les valeurs imputées
  data <- data %>%
    mutate(y = car + heavy)
  
  res_imput <- data.frame(date = as.Date(character()), y_hat = numeric(), imputed = character())
  
  # Boucle sur les combinaisons uniques de weekday et hour
  for (d in unique(data$weekday)) {
    for (h in unique(data$hour)) {
      ...
      # Vérifier s'il y a des données à imputer
      if (nrow(data_hour) > 0) {
        # Appliquer la moyenne mobile avec zoo::rollapply
        data_hour <- data_hour %>%
          mutate(y_hat = rollapply(y, width = window, FUN = function(x) mean(x, na.rm = TRUE), fill = NA, align = "center"))
        
        # Ajouter l'indication d'imputation

        
        # Ajouter les résultats au dataframe des imputations
        res_imput <- rbind(res_imput, data_hour %>% select(date, y_hat))
      }
    }
  }
  
  # Fusionner les données imputées avec les données d'origine
  data <- merge(data, res_imput, by = "date", all.x = TRUE) %>%
    mutate(imputed = as.factor(case_when(
      is.na(y) & !is.na(y_hat) ~ "imputed",
      is.na(y) & is.na(y_hat) ~ "not imputed",
      !is.na(y) ~ "original"
    ))) %>% 
    mutate(y_hat = ifelse(imputed == "original", y, y_hat))
...
# Charger les données
sensor <- retrieve_missing_data(data_04,successive_day=2)

res <- rolling_hourly(sensor,20)
table(res$imputed)


#Représenter graphiquement les valeurs imputés, changer la couleur des points; mais n'avoir qu'une unique ligne et représenter egalement la ou les valeurs ne sont pas imputés les dates correspondantes 

#Afficher serie brut
ggplot(sensor) + 
  geom_point(aes(x=date,y=car+heavy))
...
ggplot(res) +
  ...
  scale_color_manual(values = c("original" = "blue", "imputed" = "green")) +
  theme_minimal()


res %>% filter(imputed == "imputed") %>% ggplot() +
  geom_point( aes(x = date, y = y_hat, color = imputed)) +
  geom_line( aes(x = date, y = y_hat)) +
  labs(
    title = "Imputation des valeurs manquantes",
...
#Compter le pourcentage de données imputés et non imputés
res %>% group_by(imputed) %>% summarise(n = n()) %>% mutate(percentage = n / sum(n) * 100)
#
