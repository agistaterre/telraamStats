...
```{r}
data_filled <- zoo::na.approx(Burel_train$y)
...
#Algo PELT sur Burel
library(changepoint)
cptmean <- cpt.mean(data_filled, method = "PELT")
...
cpts_mean <- cpts(cptmean)
cpts_var <- cpts(cptvar)
cpts_meanvar <- cpts(cptmeanvar)


#Recuperer les dates relatifs a ses changements
date_mean <- Burel_train$ds[cpts_mean]
date_var <- Burel_train$ds[cpts_var]
date_meanvar <- Burel_train$ds[cpts_meanvar]
...
# Remplacer les NA par la moyenne des non-NA (ou une autre méthode de votre choix)
data_filled <- zoo::na.approx(Burel_train$y)

# Appliquer la méthode de détection des points de rupture
breakpoints_result <- breakpoints(data_filled ~ 1)

# Résumé des résultats
summary(breakpoints_result)

# Visualiser les points de rupture
plot(breakpoints_result)
...
```{r}
#Je veux représenter la difference lors du changement de point de vue entre le signal global brut, le signal journalier et le signal horaire pour voir la difference de réprésentation, represente graphiquemenrt


# Représenter les données brutes
G1 <- data_imput %>% ggplot(aes(x = ds, y = y)) +
  geom_point() +
  ggtitle("Données brutes") +
  xlab("Date") +
  ylab("Nombre de véhicules")

# Représenter les données journalières colorées par jour de la semaine
G2 <- data_imput %>% ggplot(aes(x = ds, y = y, col = weekday)) +
  geom_point() +
  ggtitle("Données différenciées par jour") +
  xlab("Date") +
  ylab("Nombre de véhicules") +
  theme(legend.position = "none")

# Représenter les données des lundis
G2bis <- data_imput %>% filter(weekday == "Monday") %>% ggplot(aes(x = ds, y = y)) +
  geom_point() +
  ggtitle("Données des lundis") +
  xlab("Date") +
  ylab("Nombre de véhicules")


# Représenter les données par jour et par heure
G3 <- data_imput %>% filter(weekday == "Monday") %>% ggplot(aes(x = ds, y = y, col = as.character(hour))) +
  geom_point() +
  ggtitle("Données par jour et heure") +
  xlab("Date") +
  ylab("Nombre de véhicules") +
  theme(legend.position = "none")

# Zoom sur les lundis à 8h
G3bis <- data_imput %>% filter(weekday == "Monday", hour == 8) %>% ggplot(aes(x = ds, y = y)) +
  geom_point() +
  ggtitle("Lundi 8h") +
  xlab("Date") +
  ylab("Nombre de véhicules")
...
grid.arrange(
  ...
  G3, G3bis, 
  ncol = 3, 
  layout_matrix = rbind(c(NA, 2, 4), c(1, 3, 5))
)
