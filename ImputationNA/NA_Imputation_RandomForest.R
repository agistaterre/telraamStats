...
impute_missing_data <- function(data, sensors_name = NULL, transport_type = "vehicle") {
  ...
  if (!"week_number" %in% colnames(data)) {
    data$week_number <- week(data$date)
  }
...
# Example usage:
# Impute missing data for the transport type 'vehicle', 'car'

start_time <- Sys.time()
dat <- impute_missing_data(data_imput, transport_type = "car")
print(names(dat))
end_time <- Sys.time()
print(end_time - start_time)

dat <- impute_missing_data(data_imput, transport_type = "vehicle")
print(dat)

dat <- impute_missing_data(data_imput, transport_type = "heavy")
print(dat)

dat <- impute_missing_data(data_imput, transport_type = "all")
print(dat)


#Try to impute missing data where column hour is missing
d_test <- data_imput %>% select(-hour)
dat <- impute_missing_data(d_test, transport_type = "vehicle")
print(dat)

#Try to impute missing data where transport_type is wrong
dat <- impute_missing_data(data_imput, transport_type = "wrong")
print(dat)
