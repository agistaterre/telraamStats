...
#' @param aggregated_by Character. Options are : 'segment_name', 'weekday'. Default : 'weekday'.
#'
...
gg_traffic_avg <- function(enriched_data,
  ...
  agg_level = check_options_graph(aggregated_by, c('segment_name','weekday'),'weekday')
  ...
  # Ordering weekdays
  if(aggregated_by == "weekday"){
    traffic$weekday <- ordered(traffic$weekday, levels = c("lundi","mardi","mercredi","jeudi",
                                                           "vendredi","samedi","dimanche"))
  }

  # Differentiating segment names
  if(aggregated_by == "segment_name"){
    traffic <- traffic %>% unnest(cols = .data$segment_name)
  }

  # Graph
  graph <- ggplot(traffic, aes(x = hour, y = .data$traffic_sum, color = as.factor(.data[[aggregated_by]]))) +
    geom_line() +
