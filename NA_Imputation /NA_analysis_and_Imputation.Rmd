...
##Nettoyage avec stop_sensor 
```{r}
sensor_clear <- data.frame()
n1 <- 0
n <- 0
for (i in 1:length(segments)){
   sensor_clear <- rbind( sensor_clear,stop_sensor(segments[[i]],uptime_choice=0.5))
}
...
cor_sensor <- function(df) {
  ...
  mutate(across(where(is.list), ~map_dbl(.x, ~ifelse(length(.x) == 0, NA, .x[[1]]))))

  sensors <- setdiff(names(df), "date")
  cor_matrix <- matrix(NA, nrow = length(sensors), ncol = length(sensors), 
                       dimnames = list(sensors, sensors))
  
  # Parcourir tous les capteurs pour remplir la matrice de corrélation
  for (sensor1 in sensors) {
    df_sensor1 <- df %>% select(date, all_of(sensor1)) %>% na.omit()
    
    for (sensor2 in sensors) {
      if (sensor1 == sensor2) {
        cor_matrix[sensor1, sensor2] <- 1
      } else {
        df_sensor2 <- df %>% select(date, all_of(sensor2)) %>% na.omit()
        df_merged <- merge(df_sensor1, df_sensor2, by = "date")
        
        if (nrow(df_merged) > 1) {
          correlation <- cor(df_merged[[sensor1]], df_merged[[sensor2]], use = "complete.obs")
          cor_matrix[sensor1, sensor2] <- correlation
        }
      }
    }
  }
...
cor_sensor(segments_clear) %>% corrplot::corrplot( method = "number", type = "upper", tl.col = "black", tl.srt = 45)   

#dataV2 <- dataV2 %>% rename(segment_name = segment_fullname)
dataV2 <- dataV2  %>% filter(segment_name != "") %>%  mutate(segment_name = as.factor(as.character(gsub("[0-9-]+", "",segment_name))),uptime = as.numeric(uptime))
cor_sensor(dataV2) %>% corrplot::corrplot( method = "number", type = "upper", tl.col = "black", tl.srt = 45)   
...
ggplot(Leclerc, aes(x = date, y = vehicle)) +
  geom_line() +
  geom_point(data = chow_results %>% filter(is_rupture), aes(color = "Rupture Period"), size = 2) +
  labs(title = "Detection of Unusual Periods in Time Series",
       x = "Date",
       y = "Vehicle Count") +
  scale_color_manual(values = c("Rupture Period" = "red")) +
  theme_minimal()
```
...
TimeSeries_Imputation <- function(data) {
  ...
  model <- prophet(train_set, holidays = holidays,seasonality.prior.scale =0.1)
